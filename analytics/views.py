from django.http import JsonResponse
from django.shortcuts import render
from django.db.models import Count, Q
from django.db.models.functions import TruncMonth   # ← add this
from django.utils import timezone
from datetime import timedelta

from employees.models import Department
from attendance.models import Attendance

def charts_page(request):
    return render(request, "charts.html")

def employees_per_department(request):
    data = (Department.objects.annotate(total=Count("employees"))
            .values_list("name", "total"))
    return JsonResponse({"labels": [n for n, _ in data], "values": [t for _, t in data]})

def monthly_attendance(request):
    today = timezone.now().date()
    start = (today.replace(day=1) - timedelta(days=180)).replace(day=1)

    qs = (Attendance.objects
          .filter(date__gte=start)
          .annotate(month=TruncMonth("date"))              # ← use TruncMonth here
          .values("month")
          .annotate(present=Count("id", filter=Q(status=Attendance.STATUS_PRESENT)))
          .order_by("month"))

    labels = [row["month"].strftime("%Y-%m") for row in qs]  # ← month key
    values = [row["present"] for row in qs]
    return JsonResponse({"labels": labels, "values": values})
